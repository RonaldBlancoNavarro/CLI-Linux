...
{   //seleccion de comando segun indice en historial
...
void ejecutarArgsSimples(char **argAnalizado)
{
    ...
    {
        // El padre espera a que el hijo termine la instrucción
        wait(NULL);
        return;
    }
}
...
void ejecutarArgsPipe(char **argAnalizado, char **argPipe)
{
    ...
    else
    {
        ...
        {
            // Cerrar la escritura del proceso padre
            close(tuberiaRW[1]);
            
            // El padre espera a que terminen de ejecutarse los hijos
            wait(NULL);
            wait(NULL);
        }
    }
